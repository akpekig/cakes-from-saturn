...
const toppingsWithAmount = [
    "CHERRY"
]

const toppingsWithColor = [
    "HEARTS",
    "SPRINKLES"
]
    ...
    const toppingsComponents = useMemo(() => {
        return sortedToppings.map((topping, index) => {
            ...
            if (toppingsWithAmount.includes(type)) {
                return (<Suspense key={index} fallback={null}><Component amount={topping.amount ?? "ONE"}/></Suspense>)
            }

            if (toppingsWithColor.includes(type)) {
                return (<Suspense key={index} fallback={null}><Component color={topping.color ?? "RAINBOW"}/></Suspense>)
            }

            if (toppingsWithChocolate.includes(type)) {
                return (<Suspense key={index} fallback={null}><Component /></Suspense>)
            }
        })
    }, [])
    return (
